...
(() => {
  ...
  function calculateStatBonus(stat, value, level = 1) {
    ...
    switch(stat) {
      ...
      case 'luck':
        return `${Math.round(getCritChanceFromLuckAndLevel(safe, lvl) * 100)}% Crit`;
      default:
        return '';
    }
  }
...
function sfGetTooltipDelayMs(){
  ...
  function showTooltip(item, x, y) {
    ...
    const rarityLabel = ({common:'⚪ Common',rare:'🔵 Rare',epic:'🟣 Epic',legendary:'🟠 Legendary'}[item.rarity]||'⚪ Common');
    tooltip.innerHTML = `
      <div class="tt-head">
        <div class="tt-title">${icon}<span class="tt-name">${name}</span></div>
        <div class="tt-sub">
  ...
  function calculateTotalBonuses(row) {
    const bonuses = { strength: 0, defense: 0, constitution: 0, luck: 0 };
  ...
  async function loadLeaderboard(type = 'level', forceRefresh = false) {
    try {
      ...
      let query = sb
        .from('player_stats')
        // stats obsahují character_name + avatar_url + avatar_frame + player_class
        .select('user_id, level, xp, money, stats');
      ...
      const sanitizedData = (data || []).map(player => {
        // stats můžou být objekt nebo JSON string
        let st = player.stats || {};
        if (typeof st === 'string') {
  ...
  function renderLeaderboard(players, type = 'level') {
    ...
    players.forEach((player, index) => {
      ...
      const st = (player && player.stats && typeof player.stats === 'object') ? player.stats : {};
      const charName = st.character_name ? String(st.character_name).trim() : '';
      const playerName = isMe
        ? (charName || 'TY (BORIS GOPNIKOV)')
        : (charName || `PLAYER ${player.user_id.substring(0, 8).toUpperCase()}`);

      const avatarUrl = st.avatar_url ? String(st.avatar_url) : 'avatar.jpg';
      const frameColor = st.avatar_frame ? String(st.avatar_frame) : null;
      
      item.innerHTML = `
        <div class="rank">${medal}</div>
        <div class="lb-avatar" style="border-color:${frameColor ? frameColor : 'rgba(201,164,74,.55)'}">
          <img src="${avatarUrl}" alt="avatar" loading="lazy" onerror="this.src='avatar.jpg'; this.onerror=null;">
        </div>
  ...
  function updatePlayerView(data) {
    ...
    // Avatar + frame + badge (podle hráče, ne podle tebe)
    try {
      const avatarEl = document.getElementById('playerAvatar');
      if (avatarEl && st.avatar_url) avatarEl.src = String(st.avatar_url);

      const charBox = avatarEl ? avatarEl.closest('.character') : document.querySelector('#playerView .character');
      if (charBox && st.avatar_frame) charBox.style.borderColor = String(st.avatar_frame);

      const clsMap = { padouch: '👻', rvac: '✊', mozek: '💡' };
      const cls = st.player_class ? String(st.player_class).toLowerCase() : '';
      if (charBox) {
        let b = charBox.querySelector('.class-badge');
        if (!b) {
          b = document.createElement('div');
          b.className = 'class-badge';
          charBox.appendChild(b);
        }
        b.textContent = clsMap[cls] || '👻';
    ...
    ALLOWED_STATS.forEach(stat => {
      ...
      const extraEl = document.getElementById(`stat${stat.charAt(0).toUpperCase() + stat.slice(1)}Extra`);
	      const bonusEl = document.getElementById(`stat${stat.charAt(0).toUpperCase() + stat.slice(1)}Bonus`);
      
	      if (statEl) statEl.textContent = formatStatValue(totalValue);
	      if (extraEl) {
	        const extra = calculateStatBonus(stat, totalValue, lvl, (st.player_class?String(st.player_class).toLowerCase():'padouch'));
	        extraEl.textContent = extra;
	      }
	      if (bonusEl) bonusEl.textContent = `🐾 +${fmtPct(p)}%  👥 +${fmtPct(g)}%`;
    });
  ...
  async function showPlayerView(player) {
    try {
      ...
      let st = playerData.stats || {};
      if (typeof st === 'string') {
        try { st = JSON.parse(st); } catch { st = {}; }
      }
      const characterName = st?.character_name;
      const playerName = isMe 
