...
(() => {
  ...
  class SupabaseManager {
    ...
    // Donate do trezoru guildy (best-effort: různé schémata)
    // type: 'money' | 'cigs'
    static async donate(guildId, type, amount) {
      ...
      try {
        ...
        console.warn('[guild] donate: update guild_members failed:', e);
      }

      // 2) zkus uložit donation log (pokud tabulka existuje)
      try {
        await sb
          .from('guild_donations')
          .insert([{ guild_id: guildId, user_id: userId, type, amount: amt, created_at: new Date().toISOString() }]);
      } catch (e) {
        // tabulka nemusí existovat → ignor
      }

      // 3) zkus navýšit trezor přímo na guilds tabulce (různé názvy sloupců)
      const moneyCols = ['bank_money', 'treasury_money', 'money_bank', 'vault_money', 'money'];
      const cigsCols = ['bank_cigs', 'treasury_cigs', 'cigs_bank', 'vault_cigs', 'cigarettes'];
      const cols = (type === 'money') ? moneyCols : cigsCols;

      let guildRow = null;
      try {
        const { data } = await sb.from('guilds').select('*').eq('id', guildId).maybeSingle();
        guildRow = data || null;
      } catch {}

      for (const col of cols) {
        try {
          const current = Number(guildRow?.[col] ?? 0);
          ...
          if (msg.includes('could not find') || msg.includes('column') || e?.code === 'PGRST204') {
            continue;
          }
          // jiná chyba → vyhoď
          throw e;
      ...
      // když nemáme žádný trezor sloupec, aspoň donation log + cooldown proběhl
    }
