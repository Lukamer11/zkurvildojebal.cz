...
function compute(state) {
  ...
  // Globální balancing: alespoň +30% výnos (cigarety/klik a pasiv) oproti původnímu.
  const balanceBonus = 1.30;
  
  const baseCpc = 0.0000010 + state.clickLevel * 0.0000010;
  const cpc = baseCpc * prestigeBonus * state.combo * balanceBonus;
  
  const baseCps = state.cursor * 0.0000002 + state.granny * 0.000002;
  const cps = baseCps * prestigeBonus * balanceBonus;
...
function render() {
  ...
  if (critEl) {
    const luck = Number(window.SF?.stats?.stats?.luck || window.SF?.stats?.luck || 0);
    const critPct = Math.round(Math.max(0, Math.min(50, luck * 0.25)));
    critEl.textContent = critPct + '%';
  }
...
function onClick() {
  ...
  // Sound odstraněn (bez hudby při kliku)
  ...
  // Crit (ze štěstí hráče)
  const luck = Number(window.SF?.stats?.stats?.luck || window.SF?.stats?.luck || 0);
  const critChance = Math.max(0, Math.min(0.50, luck * 0.0025));
